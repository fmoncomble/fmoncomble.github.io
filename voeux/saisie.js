document.addEventListener('DOMContentLoaded', async () => {
	...
	function buildCourseList() {
		...
		for (let c of courseChoiceList) {
			...
			let existingOption = jsonFile?.Cours?.find(
				(cours) => cours.id === c.id,
			);
			if (existingOption) {
	...
	async function saveCourse(course, value, mode) {
		...
		// entryNb.value = 1;
		const entryNb = entry.querySelector('select.added-course-item-nb');
		const entryNbMax = course.nbgrp;
		for (let i = 1; i <= entryNbMax; i++) {
			const entryNbOption = document.createElement('option');
			entryNbOption.value = i;
			entryNbOption.textContent = i;
			entryNb.appendChild(entryNbOption);
		}
